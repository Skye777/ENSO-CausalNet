...
ninoweight = ninoweight[:args.prediction_steps]
...
def nino_loss(y_pred, y_true):
    rmse = torch.sqrt(torch.mean((y_pred - y_true) ** 2, dim=0))
    return rmse.sum()

# 重参数技巧
def sampling(z_mean, z_log_var):
    epsilon = torch.randn_like(z_mean)  # 使用与z_mean相同的shape
...
def gumbel_softmax_sample(logits, tau=1, eps=1e-10):
    ...
    if logits.is_cuda:
        gumbel_noise = gumbel_noise.to(args.device)
...
def kl_loss(z_mean, z_log_var):
    return -0.5 * torch.sum(1 + z_log_var - z_mean**2 - torch.exp(z_log_var)) / z_mean.size(0)
    # return -0.5 * torch.sum(1 + torch.log(z_std**2) - z_mean**2 - z_std**2) / z_mean.size(0)
...
def create_rel_rec_send(args, num_atoms):
    ...
    if args.cuda:
        rel_rec = rel_rec.to(args.device)
        rel_send = rel_send.to(args.device)
...
def get_uniform_logprobs(inferred_mu, inferred_width, temperatures):
    ...
    probs = torch.where(
        cdf * (1 - cdf) > 0.0, log_prob_default, torch.full(cdf.shape, -8).to(args.device)
    )
    return probs.mean()
